...
phe_results <- as.data.frame(read.table("../output/hla_combined_bma_snp.tsv", header=TRUE))

phe_results <- phe_results %>% filter(GBE_ID == phe_name)

if (nrow(phe_results) < 2) {
    print("Not enough significant results to run BMA!")
} else {
    print("Reading in dosage file...")
    hla <- readRDS("../hla_genotype_data/ukb_hla_v3_24983_rounded_wb.rds")
    snp <- readRDS("../hla_genotype_data/hla_snp_wb.rds")
    dosage <- cbind(hla, snp)
    ...
    all_names = phe_results$allelotype
    print("Running BMA for allelotypes/SNPs: ")
    print(all_names)
    ...
    # run BMA analysis
    if (analysis_type == "BIN") {
        glm.out.FF <- bic.glm(input, pheno, strict = FALSE, OR = 5, glm.family="binomial", factor.type=FALSE, maxCol=dim(input)[[2]])
    } else {
        glm.out.FF <- bic.glm(input, pheno, strict = FALSE, OR = 5, glm.family="gaussian", factor.type=FALSE, maxCol=dim(input)[[2]])
    }
    ...
    print(paste0("/oak/stanford/groups/mrivas/users/guhan/repos/hla-assoc/output/bma_snp/hla_bma_snp_", phe_name))
    write.table(results_df, paste0("/oak/stanford/groups/mrivas/users/guhan/repos/hla-assoc/output/bma_snp/hla_bma_snp_", phe_name, "_","round_all",".tsv"), col.names=NA, quote=FALSE, sep='\t')
    pdf(paste0("/oak/stanford/groups/mrivas/users/guhan/repos/hla-assoc/output/bma_snp/hla_bma_snp_", phe_name, "_round_adjp_all",".pdf"))
